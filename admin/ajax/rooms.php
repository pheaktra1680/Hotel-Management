...
//     else{
//         $flag = 0;
//         die('qurey cannot be prepared - insert');
//     }
...
if (isset($_POST['add_room'])) {
    ...
    if (insert($q1, $values, 'siiiiis')) {
        ...
        if ($stmt) {
            ...
            die('Query cannot be prepared: Facilities');
        }
        ...
        if ($stmt) {
            ...
            die('Query cannot be prepared: Features');
        }
        ...
        $data .= "
            <tr class='align-middle'>
                ...
                <td>
                    ...
                    <button type='button' onclick='room_images($row[id], \"" . addslashes(htmlspecialchars($row['name'])) . "\")' class='btn btn-info btn-sm shadow-none' data-bs-toggle='modal' data-bs-target='#room-images'>
...
if (isset($_POST['edit_room'])) {
    ...
    $del_features = custom_delete("DELETE FROM `room_features` WHERE `room_id`=?", [$frm_data['room_id']], 'i');
    $del_facilities = custom_delete("DELETE FROM `room_facilities` WHERE `room_id`=?", [$frm_data['room_id']], 'i');
    ...
    if($stmt = mysqli_prepare($con,$q2))
    {
        ...
        $flag = 0;
        die('queery cannot be prepared! - insert');
    }
...
if (isset($_POST['thumb_image'])) {
    ...
    // Reset all thumbnails for the given room
...
if (isset($_POST['remove_room'])) {
    ...
    $res2 = select("SELECT * FROM `room_images` WHERE `room_id`=?", [$frm_data['room_id']], 'i');
    $res3 = select("SELECT * FROM `room_features` WHERE `room_id`=?", [$frm_data['room_id']], 'i');
    $res4 = select("SELECT * FROM `room_facilities` WHERE `room_id`=?", [$frm_data['room_id']], 'i');
